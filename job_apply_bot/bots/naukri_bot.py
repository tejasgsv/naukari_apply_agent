"""Naukri bot (Playwright-based, Phase 7 implementation).
...
class NaukriBot(PlatformBot):
    platform = "naukri"

    # No fixed selectors; keep a set of candidate selectors.
    _CARD_SELECTORS = [
        "a[data-job-id]",
        "div[data-job-id]",
        "a.jobtuple-jobcard",
    ...
    def search_jobs(self, role: str, filters: Dict) -> Iterable[object]:
        # This Phase 7 bot exposes run(page, ...) only.
        return []
    ...
    def _build_search_url(self, role: str, start: int) -> str:
        # Naukri uses query params; keep minimal.
        keywords = role.replace(" ", "+")
        return (
            "https://www.naukri.com/" 
            f"{'' if not keywords else ''}"
            f"jobs-search?searchType=normal&searchBy=title&searchPhrase={keywords}&start={start}"
        )

    def _extract_candidate_texts(self, el) -> Tuple[str, str, str]:
        # Return (title, company, time_text)
        title = ""
        company = ""
        time_text = ""
        try:
            ...
            pass
        # Best-effort: also scan nearby/parent text.
        try:
            parent = el
            if hasattr(el, "locator"):
                parent = el
            time_text = (el.get_attribute("data-job-date") or "").strip()
        except Exception:
            pass
        return title, company, time_text

    def _is_recent_24h(self, page: Page, card_el) -> bool:
        # Best-effort: look for '24 hours', '1 day', 'today', etc.
        try:
    ...
    def _print_job_ai(self, job: dict, decision_payload: dict) -> None:
        ...
        print(
            f"[AI] title={title} company={company} AI decision={ai_decision} match_score={match_score} reason={reason}"
        )
    ...
    def _apply_best_effort(self, page: Page, job: dict, behavior: BehaviorEngine, signature: str) -> str:
        # Try to find an "Apply" button within the current job detail pane.
        # Since selectors are unknown, we look for common button texts.
        try:
            # Scroll a bit
            behavior.slow_scroll(page)

            # Candidate apply buttons
            candidates = [
                page.locator("text=/apply/i"),
            ...
            for loc in candidates:
                try:
                    if loc.first.is_visible():
                        ...
                        # Naukri may require login/captcha/manual; we treat completion as manual.
    ...
    def run(
        ...
        # Build postings list by scraping multiple pages.
        postings: List[dict] = []
        for page_num in range(max_pages):
            ...
            # Collect job cards by candidate selectors; fallback to anchors containing '/jobs/'
            card_els = []
            ...
            # De-duplicate by object identity isn't stable; dedupe by extracted href when possible.
            seen_urls = set()
            for el in card_els:
                try:
                ...
                # Time filter
                if not self._is_recent_24h(page, el):
                    continue

                # Extract title/company from element text as best-effort
                try:
                ...
                # Heuristic split: title first line, company second line if present
        ...
        for job in postings:
            try:
                ...
                if decision.get("decision") != "APPLY":
                    ...
                    if reason:
                        print(
                            f"[SKIP] {reason} title={title} company={company} match_score={match_score}"
                        )
                    else:
                        print(f"[SKIP] ai_skip title={title} company={company} match_score={match_score}")
                ...
                # Navigate to job details (best-effort)
                try:
                    if job_url:
                        page.goto(job_url)
                except Exception:
                ...
                status = self._apply_best_effort(page, job, behavior, signature)
    ...
    def apply(self, job: object, application_plan: object, behavior: object) -> object:
        raise NotImplementedError(
            "This NaukriBot expects page-aware application via run(page, ...)."
        )
